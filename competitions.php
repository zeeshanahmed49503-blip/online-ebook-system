...
if (!empty($user_name) && !empty($user_id)) {
    ...
    // 2. Check Story Submission
    $check_story = "SELECT id FROM submissions WHERE user_id = '$safe_user_id' AND competition_type = 'story'";
    $story_result = mysqli_query($conn, $check_story);
    if (mysqli_num_rows($story_result) > 0) {
        $already_submitted_story = true;
    }
}
...
<body>
    ...
    <div class="comp-content">
        ...
        <p class="comp-desc">Access the new section where you can dive straight in, show off your skills, and write all your awesome, cool stuff.</p>
    ...
    <div class="comp-content">
        ...
        <div class="comp-meta">
            <span><i class="fa-regular fa-calendar-check"></i> <strong>Status:</strong> Live Now</span>
            <span><i class="fa-solid fa-clock"></i> <strong>Deadline:</strong> 1 Week Max (No late entries)</span>
        ...
        <?php if (isset($_SESSION['name'])): ?>
            ...
            <?php else: ?>
                <a href="javascript:void(0)" id="participate-story-btn" class="comp-btn btn-primary">
                    Participate Now <i class="fa-solid fa-arrow-right-long"></i>
                </a>
            <?php endif; ?>
        <?php else: ?>
...
<input type="text" id="story-title" name="story_title" 
       ...
       style="border-radius: 12px; border: 1px solid #dcdcdc; padding: 15px; font-size: 1rem; color: #111111; width: 100%; box-sizing: border-box; transition: all 0.3s ease; box-shadow: inset 0 2px 4px rgba(0,0,0,0.02);">                </div>
...
if (essayTextarea) {
    ...
    function lockAllInputs() {
        ...
        if (textarea) { textarea.disabled = true; textarea.style.background = "#eeeeee"; }
        if (fileInput) { fileInput.disabled = true; var dropZone = document.querySelector('.file-drop-zone'); if (dropZone) dropZone.style.pointerEvents = "none"; }
    ...
    function openStoryWorkspace() {
        ...
        storyWorkspace.scrollIntoView({ behavior: 'smooth' });
    }
    ...
    function startCountdown() {
        ...
        if (savedTime === 'expired') { lockAllInputs(); return; }

        var totalSeconds = parseInt(savedTime);
        if (isNaN(totalSeconds) || totalSeconds <= 0) { totalSeconds = 10800; }
        ...
        timerInterval = setInterval(function () {
            if (totalSeconds <= 0) {
                ...
                lockAllInputs();
                setTimeout(function() { alert("Time Over! Workspace locked."); }, 50);
                return; 
            }
            totalSeconds--;
